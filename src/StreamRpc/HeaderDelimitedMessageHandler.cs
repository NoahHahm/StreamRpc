...
namespace StreamRpc
{
    ...
    public class HeaderDelimitedMessageHandler : PipeMessageHandler
    {
        ...
        private const string DefaultSubType = "msgpack";
        ...
        /// <summary>
        /// The <see cref="IBufferWriter{T}"/> sent to the <see cref="MessageHandlerBase.Formatter"/> to write the message.
        /// </summary>
        private readonly Sequence<byte> contentSequenceBuilder = new Sequence<byte>(ArrayPool<byte>.Shared);

        /// <summary>
        /// Backing field for <see cref="Encoding"/>.
        /// </summary>
        ...
        {
            if (formatter is IJsonRpcMessageTextFormatter f)
                this.encoding = f.Encoding;
            if (this.encoding == null)
                this.encoding = DefaultContentEncoding;
        }
        ...
        public HeaderDelimitedMessageHandler(Stream sendingStream, Stream receivingStream)
            : this(sendingStream, receivingStream, new MessagePackFormatter())
        {
        }
        ...
        public Encoding Encoding
        {
            ...
            {
                this.encoding = value;
                if (this.Formatter is IJsonRpcMessageTextFormatter formatter)
                    formatter.Encoding = value;
            }
        }
        ...
        protected override async ValueTask<JsonRpcMessage> ReadCoreAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                else if (this.SubType == DefaultSubType)
                {
        ...
        protected override void Write(JsonRpcMessage content, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Verify.Operation(Utf8Formatter.TryFormat(contentSequence.Length, headerMemory.Span.Slice(bytesWritten), out int formattedBytes));
        ...
        private static unsafe Encoding ParseEncodingFromContentTypeHeader(ReadOnlySequence<byte> contentTypeValue)
        {
            ...
            try
            {
                ...
                if (mediaType.CharSet != null)
                {
                    ...
                    // accepted here so StreamRpc can be used to support remote language servers following common language protocol.
