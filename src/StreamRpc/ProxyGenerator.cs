...
namespace StreamRpc
{
    ...
    public class ProxyGenerator
    {
        ...
        private static readonly AssemblyName ProxyAssemblyName = new AssemblyName(string.Format(CultureInfo.InvariantCulture, "StreamRpc_Proxies_{0}", Guid.NewGuid()));
        ...
        private readonly ModuleBuilder proxyModuleBuilder;
        private readonly Dictionary<TypeInfo, TypeBuilder> generatedProxyBuildersByInterface = new Dictionary<TypeInfo, TypeBuilder>();
        private readonly Dictionary<TypeInfo, TypeInfo> generatedProxiesByInterface = new Dictionary<TypeInfo, TypeInfo>();
        ...
        internal TypeBuilder GetProxyBuilder(TypeInfo serviceInterface)
        {
            ...
            lock (this.generatedProxyBuildersByInterface)
            {
                if (this.generatedProxyBuildersByInterface.TryGetValue(serviceInterface, out proxyBuilder))
                {
                    return proxyBuilder;
                }

                proxyBuilder = GenerateTypeBuilder(serviceInterface);
                this.generatedProxyBuildersByInterface.Add(serviceInterface, proxyBuilder);
            }
        ...
        internal TypeInfo Get(TypeInfo serviceInterface)
        {
            ...
            {
                if (this.generatedProxiesByInterface.TryGetValue(serviceInterface, out generatedType))
                {
                    return generatedType;
                }

                var builder = GetProxyBuilder(serviceInterface);
                generatedType = builder.CreateTypeInfo();
                this.generatedProxiesByInterface.Add(serviceInterface, generatedType);
            }
#if SaveAssembly
            this.assemblyBuilder.Save(this.proxyModuleBuilder.ScopeName);
            System.IO.File.Move(this.proxyModuleBuilder.ScopeName, this.proxyModuleBuilder.ScopeName + ".dll");
#endif
            return generatedType;
        }

        private TypeBuilder GenerateTypeBuilder(TypeInfo serviceInterface)
        {
            var methodNameMap = new JsonRpc.MethodNameMap(serviceInterface);

            var interfaces = new List<Type> { serviceInterface.AsType(), };

            interfaces.Add(typeof(IJsonRpcClientProxy));

            var proxyTypeBuilder = this.proxyModuleBuilder.DefineType(
            ...
            foreach (var evt in FindAllOnThisAndOtherInterfaces(serviceInterface, i => i.DeclaredEvents))
            {
                ...
                var eventArgsType = evt.EventHandlerType.GetTypeInfo()
                                       .GetDeclaredMethod(nameof(EventHandler.Invoke))
                                       .GetParameters()[1]
                                       .ParameterType;
            ...
            // IJsonRpcClientProxy.JsonRpc property
            {
                ...
                // get_JsonRpc() method
                var jsonRpcPropertyGetter = proxyTypeBuilder.DefineMethod("get_" + nameof(IJsonRpcClientProxy.JsonRpc), MethodAttributes.Private | MethodAttributes.HideBySig | MethodAttributes.NewSlot | MethodAttributes.Virtual | MethodAttributes.SpecialName, typeof(JsonRpc), Type.EmptyTypes);
        ...
        private ConstructorInfo CreateParameterObjectType(ParameterInfo[] parameters, Type parentType)
        {
        ...
        private void ImplementRaiseEventMethod(ILGenerator il, FieldBuilder evtField, FieldBuilder jsonRpcField)
        {
        ...
        private void ImplementEventAccessor(ILGenerator il, FieldInfo evtField, MethodInfo combineOrRemoveMethod)
        {
        ...
        private void VerifySupported(bool condition, string messageFormat, MemberInfo problematicMember, params object[] otherArgs)
        {
            if (!condition)
            {
        ...
        private IEnumerable<T> FindAllOnThisAndOtherInterfaces<T>(TypeInfo interfaceType, Func<TypeInfo, IEnumerable<T>> oneInterfaceQuery)
        {
